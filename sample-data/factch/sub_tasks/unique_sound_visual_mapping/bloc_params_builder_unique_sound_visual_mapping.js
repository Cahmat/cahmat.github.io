...
function buildBlocParams_unique_sound_visual_mapping(params, bloc_params) {
	...
	for (i = 0; i < params.size_dimensions; i++) {
		for (j = 0; j < params.size_dimensions; j++) {
			list_imgs.push(params.stim_set_path_img + params.stim_set_img + '/set_0/' + i + j + '.png')
		}
	}
	list_imgs = jsPsych.randomization.repeat(list_imgs, 1)
	...
	for (i = 0; i < params.size_dimensions; i++) {
		for (j = 0; j < params.size_dimensions; j++) {
			img = {}
			img.left = params.width / 2 + (-1.5 + i) * bloc_params.grid_img_size_step - bloc_params.img_size / 2
			img.top = params.height / 2 + (1.5 - j) * bloc_params.grid_img_size_step - bloc_params.img_size / 2
		...
		for (j = 0; j < params.size_dimensions; j++) {
			for (k = 0; k < n_repetitions_of_the_set; k++) {
				...
				trial = {
					...
					y: j,
					list_imgs: list_imgs,
					correct_response: params.stim_set_path_img + params.stim_set_img + '/set_0/' + i + j + '.png',
				}
		...
		for (j = 0; j < params.size_dimensions; j++) {
			for (k = 0; k < n_repetitions_of_the_set; k++) {
				...
				trial = {
					...
					y: 1,
					list_imgs: list_imgs,
					correct_response: params.stim_set_path_img + params.stim_set_img + '/set_0/' + i + 1 + '.png',
				}
				bloc_params.trials.push(trial)
			}
		}
		...
		for (j = 0; j < params.size_dimensions; j++) {
			for (k = 0; k < n_repetitions_of_the_set; k++) {
				...
				trial = {
					...
					y: j,
					list_imgs: list_imgs,
					correct_response: params.stim_set_path_img + params.stim_set_img + '/set_0/' + 1 + j + '.png',
				}
				bloc_params.trials.push(trial)
			}
		}
